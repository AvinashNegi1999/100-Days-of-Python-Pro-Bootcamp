...
# ----------------------- Screen Setup -----------------------
# TODO: Create a turtle screen window and set its title
screen = turtle.Screen()
screen.title("U.S. States Game")

# TODO: Define the path to the U.S. map image and add it as a turtle shape
...
# ----------------------- Data Setup -----------------------
# TODO: Read the CSV file containing the list of states and their coordinates
...
# TODO: Define the full path to save the missing states CSV file in the same folder
output_path = os.path.join(script_dir, "states_to_learn.csv")

# ----------------------- Main Game Loop -----------------------
# ! Run the game loop until the user has guessed all 50 states
while len(guessed_states) < 50:
    # TODO: Prompt the user to input a state name, showing current score in the title bar
    ...
    if answer_state:
        ...
        if answer_state == "Exit":
            # TODO: Create a list of states not yet guessed
            missing_states = [
                state for state in all_states if state not in guessed_states
            ]
            # TODO: Save missing states to CSV file in the script directory
        ...
        if answer_state in all_states and answer_state not in guessed_states:
            ...
            # TODO: Get the x, y coordinates of the guessed state from the CSV data
            state_data = data[data.state == answer_state]
            x = int(state_data.x)
            y = int(state_data.y)

            # TODO: Create a turtle to write the state name on the map
            marker = turtle.Turtle()
            marker.hideturtle()  # * Hide the turtle cursor
            marker.penup()  # * Lift the pen to move without drawing
            marker.goto(x, y)  # * Move to the state's coordinates
            marker.write(answer_state)  # * Write the state name
